package com.syc.salesAgent.tools;

import com.syc.salesAgent.entity.SalesOrder;
    ...
    public String queryOrders(
        ...
        try {
            ...
            if(regionName !=null && !regionName.isBlank()){
                ...
                if(regionId == null){
                    return  "未找到该大区名称:" +regionName + "，请检查名称是否正确(华东区,华南区,华北区,西南区)";
                }
            }
            //销售员名称转ID
            Long repId = null;
            if(repName !=null && !repName.isBlank()){
                ...
                if(repId == null){
                    return  "未找到该销售员名称:" +repName + "，请检查名称是否正确";
                }
            }
...
private   String formatOrders(List<SalesOrder> orders, int total,
    ...
    sb.append(String.format("订单查询结果（%s 至 %s%s）：\n",
            startDate,endDate,regionName!= null? " " + regionName : ""));
    sb.append(String.format("共 %d 条订单数据\n",total));
    if (orders.size()<total){
        sb.append(String.format("返回 %d 条数据\n",orders.size()));
    }
    sb.append("\n\n");
